...
char AUTH_SERVER_PORT[]="20001"; // authentication server port
char AUTH_SERVER_IP[]="127.0.0.1"; // auth server IP address
...
int main(int argc, char **argv)  
{
	...
	//scanf(" %[^\t\n]s",AUTH_SERVER_IP);
	//AUTH_SERVER_IP = "10.130.5.172";


	//printf("Enter Authentication server listening port  ");
	//scanf(" %[^\t\n]s",AUTH_SERVER_PORT);
	//AUTH_SERVER_PORT = "20000";
		...
		if (success)
			{
				...
				if ( (len = recv(new1, rec_buff, MAX_BUFFER, 0)) < 0) {
				perror("Error in recieve");
					}
				sscanf (rec_buff, "%s\n", file_name); /* discard CR/LF */
				bzero(rec_buff,MAX_BUFFER);
