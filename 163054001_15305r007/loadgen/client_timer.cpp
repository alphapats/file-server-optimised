...
int main(int argc, char **argv)  //IP and port mentioned
{
	...
	std::vector<double> t_resp_time;
	std::vector<double> t_data_tfred; 
	std::vector<long> complete_count; 
	...
	cout<<"Enter file name (1MB/1KB/10KB...)  : ";
	cin>>f_name;

	cout<<"Total files available  : ";
	cin>>f_count;
...
//block until all threads complete 
  for (id = 0; id < cnt; id++) {
    ...
    t_ret = *((result *)vptr); 
    t_resp_time.push_back(t_ret.total_resp_time);
    complete_count.push_back(t_ret.comp);
	error_count.push_back(t_ret.err);
   t_data_tfred.push_back(t_ret.data_tfred);
  ...
  free(vptr); 
   ...
   vector<double>::iterator v = t_resp_time.begin();
   while( v != t_resp_time.end()) {
   ...
   // use iterator to access the values
   vector<int>::iterator p= error_count.begin();
   while( p != error_count.end()) {
   		
   		error_counter=error_counter+ *p;
          p++;
   }

   // use iterator to access the values
   vector<long>::iterator q= complete_count.begin();
   while( q != complete_count.end()) {
   		
   		complete_counter=complete_counter+ *q;
          q++;
   }

   vector<double>::iterator r = t_data_tfred.begin();
   while( r != t_data_tfred.end()) {
   		
   		total_data_tfred=total_data_tfred+ *r;
          r++;
   }
	...
	//cout<<"Total MYSQL login  error" <<login_unsucc_counter<<endl;
  	...
  	cout<<"Data transfered per sec MBps" << total_data_tfred/elapsed.count() <<endl;
  	...
  	result_file<<endl<<NUM_THREADS<<"\t \t"<<mean_resp_time<<"\t \t"<<throughput<<"\t \t"<<data_exch<<"\t \t"<<complete_counter
  	<<"\t \t"<<error_counter<<"\t \t"<<RUN_TIME<<"\t \t"<<"\t \t"<<elapsed.count()<<"\t \t"<<REQUEST_TYPE;
...
double connect_to_server(void *threadid,char* REQUEST_TYPE, char* file_name)
{
	...
	while(1)
	{
				...
				{	
					char *random_wd = randomString(4);
					strcpy(newreq.uname,random_wd);
					strcpy(newreq.upassword,random_wd);
					//strcpy(newreq.uname,"user");
					//strcpy(newreq.upassword,"user");
...
while(1)
{
	...
	{
		//int rdm=rand()%1000+1;
		//int rdm2=rand()%1000+1;
		int r=(rand())%f_count+1;
...
//cout<<(long)threadid<<" Thread "<<t_complete_counter<<endl;
